...
- (void)updateAppOrientation:(UIInterfaceOrientation)orientation
{
    ...
    for(NSString* key in viewDic)
    {
        ...
        [view willRotateToOrientation: orientation fromOrientation: (UIInterfaceOrientation)UIInterfaceOrientationUnknown];
        [view didRotate];
...
- (void)willStartWithViewController:(UIViewController*)controller
{
    ...
    {
        UnityView* view = viewDic[key];
        view.contentScaleFactor   = UnityScreenScaleFactor([UIScreen mainScreen]);
        view.autoresizingMask     = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        
        UIViewController* viewCtr = viewCtrDic[key];
        viewCtr.view = view;
    }
}
...
- (void)didTransitionToViewController:(UIViewController*)toController fromViewController:(UIViewController*)fromController
{
    ...
    for(NSString* key in viewDic)
    {
        ...
        [view willRotateToOrientation: UIViewControllerInterfaceOrientation(toController) fromOrientation: ConvertToIosScreenOrientation(view.contentOrientation)];
        [view didRotate];
...
{
    UnityView* view = viewDic[@"root"];
    //UIViewController* viewCtr = viewCtrDic[@"root"];
...
- (void)createNewUI:(NSString*)key
{
    UnityView* view = [viewDic objectForKey:key];
    
    UIViewController* viewCtr = [viewCtrDic objectForKey:key];
    
    //[self willStartWithViewController: viewCtr];
    view.contentScaleFactor   = UnityScreenScaleFactor([UIScreen mainScreen]);
    view.autoresizingMask     = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    [self updateAppOrientation: ConvertToIosScreenOrientation(UIViewControllerOrientation(viewCtr))];
#endif
    ...
    ShowActivityIndicator(view, style ? [style intValue] : -1);
...
- (void)createUI
{
    UnityView*_rootView = viewDic[@"root"];
    ...
    UIViewController* _rootController = [self createRootViewController];

    if([viewCtrDic objectForKey:@"root"] == nil)
    {
        [viewCtrDic setObject:_rootController forKey:@"root"];
    }
...
#if UNITY_SUPPORT_ROTATION
    for(NSString * key in  viewCtrDic)
    {
        UIViewController* contrller = viewCtrDic[key];
        // to be able to query orientation from view controller we should actually show it.
        // at this point we finally started to show game view controller. Just in case update orientation again
        [self updateAppOrientation: ConvertToIosScreenOrientation(UIViewControllerOrientation(contrller))];
    }
#endif
...
- (void)transitionToViewController:(UIViewController*)vc
{
    for (NSString *key in viewCtrDic) {
        ...
        UIView* view = viewDic[key];
        UIViewController* contrller = viewCtrDic[key];
        
        [self willTransitionToViewController: vc fromViewController: contrller];
        ...
        [self didTransitionToViewController: vc fromViewController: contrller];
    }
}
...
- (void)interfaceWillChangeOrientationTo:(UIInterfaceOrientation)toInterfaceOrientation
{
    ...
    for (NSString *key in viewDic) {
        ...
        UnityView* view = viewDic[key];
        [view willRotateToOrientation: toInterfaceOrientation fromOrientation: fromInterfaceOrientation];
    }
}
...
- (void)notifyHideHomeButtonChange
{
    for(NSString* key in viewCtrDic)
    {
        ...
        if ([viewCtr respondsToSelector: @selector(setNeedsUpdateOfHomeIndicatorAutoHidden)])
            [viewCtr setNeedsUpdateOfHomeIndicatorAutoHidden];
    #endif
    }
}
...
- (void)notifyDeferSystemGesturesChange
{
    for(NSString* key in viewCtrDic)
    {
        UIViewController* viewCtr = viewCtrDic[key];
#if PLATFORM_IOS
        [viewCtr setNeedsUpdateOfScreenEdgesDeferringSystemGestures];
#endif
    }
}
...
- (void)checkOrientationRequest
{
    ...
    {
        UnityView* view = viewDic[key];
        UnityDefaultViewController* viewCtr = viewCtrDic[key];
        ...
        if (viewCtr.presentedViewController)
            continue;
        ...
        {
            NSUInteger rootOrient = 1 << UIViewControllerInterfaceOrientation(viewCtr);
            if (!autorotChanged && (rootOrient & EnabledAutorotationInterfaceOrientations()))
            {
                ...
                if ([viewCtr isKindOfClass: [UnityDefaultViewController class]])
                    [(UnityDefaultViewController*)viewCtr updateSupportedOrientations];
