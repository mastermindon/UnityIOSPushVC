...
@synthesize ViewDic                = viewDic;
//@synthesize rootViewController      = _rootController;
@synthesize ViewCtrDic      = viewCtrDic;
...
- (UIViewController*)AllocNewViewController:(NSString*)key
{
    if([viewCtrDic objectForKey:key] != nil)
    {
        return viewCtrDic[key];
    }

    UnityView* view = [self createUnityView];
    if([viewDic objectForKey:key] == nil)
    {
        [viewDic setObject:view forKey:key];
    }
    
    _mainDisplay = [DisplayManager Instance].mainDisplay;
    [_mainDisplay createWithWindow: nil andView: view];
    
    
    UIViewController* Controller = [self createRootViewController];
    if([viewCtrDic objectForKey:key] == nil)
    {
        [viewCtrDic setObject:Controller forKey:key];
    }
    
    [self createNewUI:key];
    
    return Controller;
}

- (void)ChangeShow:(NSString*)key
{
    if([viewDic objectForKey:key] == nil)
    {
        return;
    ...
    [_mainDisplay destroySurface];
    //[_mainDisplay reInitRendering]
...
extern "C" void UnityCleanupTrampoline()
{
    ...
    for(NSString* key in _UnityAppController.ViewDic)
    {
        UnityView* view = _UnityAppController.ViewDic[key];
...
- (NSUInteger)application:(UIApplication*)application supportedInterfaceOrientationsForWindow:(UIWindow*)window
{
    ...
    for(NSString* key in viewCtrDic)
    {
        ...
        UIViewController* _rootController = viewCtrDic[key];
        if (!_unityAppReady)
...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    UnityView* _unityView = [self createUnityView];

    viewDic = [NSMutableDictionary dictionary];
    viewCtrDic = [NSMutableDictionary dictionary];
    
    if([viewDic objectForKey:@"root"] == nil)
    {
        [viewDic setObject:_unityView forKey:@"root"];
    }

    [DisplayManager Initialize];
    _mainDisplay = [DisplayManager Instance].mainDisplay;
    [_mainDisplay createWithWindow: nil andView: _unityView];
    //[_mainDisplay createView: _unityView];

    //[self AllocNewViewController:@"Test"];
...
- (void)applicationWillEnterForeground:(UIApplication*)application
{
    ...
    if (_unityAppReady)
    {
        ...
        {
            UnityView* view =GetAppController().ViewDic[key];
...
- (void)addSnapshotViewController
{
    ...
    if (snapshotView != nil)
    {
        ...
        UIViewController* rootCtr = viewCtrDic[@"root"];
        [rootCtr presentViewController: snapshotViewController animated: false completion: nil];
...
{
    UnityDefaultViewController* viewCtr =GetAppController().ViewCtrDic[@"root"];
    return viewCtr;
}

extern "C" UIView*              UnityGetGLView()
{
    UnityView* view =GetAppController().ViewDic[@"root"];
    return view;
}

extern "C" ScreenOrientation    UnityCurrentOrientation()   {
    UnityView* view =GetAppController().ViewDic[@"root"];
    return view.contentOrientation;
